...
contract DB is Destructible {
    ...
    modifier onlyPermittedContractOrOwner() {
        require(
            permissionStatusForContract[msg.sender] || msg.sender == owner()
        );
        _;
    }
    ...
    function setAddressValue(
        bytes32 recordFieldName,
        address value
    ) external onlyPermittedContractOrOwner {
    ...
    function setUIntValue(
        bytes32 recordFieldName,
        uint value
    ) public onlyPermittedContractOrOwner {
        UIntDataStore[recordFieldName] = value;
    }

    function addUIntValue(
        bytes32 recordFieldName,
        uint value
    ) external onlyPermittedContractOrOwner {
        UIntDataStore[recordFieldName] = SafeMath.add(
            UIntDataStore[recordFieldName],
            value
        );
    }

    function subUIntValue(
        bytes32 recordFieldName,
        uint value
    ) external onlyPermittedContractOrOwner {
        UIntDataStore[recordFieldName] = SafeMath.sub(
            UIntDataStore[recordFieldName],
            value
        );
    }
    ...
    // UInt8DataStore
    function getUInt8Value(
        bytes32 recordFieldName
    ) public view returns (uint8) {
        return UInt8DataStore[recordFieldName];
    }

    function setUInt8Value(
        bytes32 recordFieldName,
        uint8 value
    ) public onlyPermittedContractOrOwner {
    ...
    function setStringValue(
        ...
        StringDataStore[recordFieldName] = "^".toSlice().concat(
            value.toSlice()
        );
    }
    ...
    function addPermittedContracts(
    ...
    function removePermittedContract(address addresses) external onlyOwner {
        permissionStatusForContract[addresses] = false;
    }

    function removePermittedContracts(
        address[] calldata addresses
    ) external onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
    ...
    function getPermittedContracts()
        external
        view
    ...
    function getObjectData(
        bytes32[] calldata recordFieldNames,
        uint8[] calldata recordFieldTypes
    ) external view returns (uint[] memory items, string memory stringItem) {
    ...
    */
        ...
        for (uint i = 0; i < objectsCount; i++) {
            for (uint j = 0; j < recordFieldTypes.length; j++) {
                ...
                if (recordFieldTypes[j] == 7) {
                    stringItem = stringItem.toSlice().concat(
                        getStringValue(recordFieldNames[r_i]).toSlice()
                    );
                    stringItem = stringItem.toSlice().concat(
                        "666--ETH-VUE--666".toSlice()
                    );
                } else {
                    items[k] = getCorrespondingIntegerValue(
                        recordFieldNames[r_i],
                        recordFieldTypes[j]
                    );
                    k++;
                }
    ...
    function getCountOfIntegerTypes(
        uint8[] memory recordFieldTypes
    ) public pure returns (uint count) {
        count = 0;
        for (uint i = 0; i < recordFieldTypes.length; i++) {
            if (recordFieldTypes[i] != 7) {
                // recordFieldType 7 is for strings
                count += 1;
            }
        }
    ...
    function getCorrespondingIntegerValue(
        bytes32 recordFieldName,
        uint8 uintType
